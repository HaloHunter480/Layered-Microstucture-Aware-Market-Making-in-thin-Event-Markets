...
class LiveExecutor:
    ...
    def place_sell_order(
        ...
        """
        Exit a position: sell `shares` tokens via a limit order at floor_price.

        Called by the exit monitor 2-4 seconds after entry when Polymarket
        has repriced. We post a limit SELL at `floor_price` (≥ entry price)
        so we never sell at a loss. The repriced bid will fill us immediately.

        Args:
            shares:      number of tokens to sell (from open_position["size"])
            floor_price: minimum acceptable sell price (set to entry_price)
        """
        floor_price = round(max(floor_price, 0.01), 2)
        shares      = round(shares, 2)

        logger.info(
            f"SELL EXIT: {shares:.2f} shares @ ≥{floor_price:.3f} | "
            f"token={token_id[:12]}..."
        )

        if shares < 8:
            logger.warning(f"SELL skipped — only {shares:.2f} shares (Polymarket min=8)")
            return TradeResult(success=False, error=f"min 8 shares required, got {shares}")

        return self.place_limit_order(token_id, "SELL", floor_price, shares)
